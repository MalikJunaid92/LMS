...
const DashboardWidgets: FC<Props> = ({ open, value }) => {
  ...
  useEffect(() => {
    ...
    if (data?.user?.last12Months && orderData?.orders?.last12Months) {
      ...
      if (userLastTwoMonths.length === 2 && orderLastTwoMonths.length === 2) {
        const usersCurrentMonth = userLastTwoMonths[1].count || 0;
        const usersPreviousMonth = userLastTwoMonths[0].count || 0;
